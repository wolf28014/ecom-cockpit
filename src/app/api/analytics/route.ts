...
export async function GET(req: NextRequest) {
  ...
  const [today, week, month, year, trend14, trend30, trend180, skuStats] = await Promise.all([
  ...
  const dailyChange = {
    ...
    profit: yesterdaySum.netProfit !== 0 ? (today.netProfit - yesterdaySum.netProfit) / Math.abs(yesterdaySum.netProfit) : 0,
  };
  const weeklyChange = {
    sales: lastWeekSum.salesAmount > 0 ? (week.salesAmount - lastWeekSum.salesAmount) / lastWeekSum.salesAmount : 0,
    profit: lastWeekSum.netProfit !== 0 ? (week.netProfit - lastWeekSum.netProfit) / Math.abs(lastWeekSum.netProfit) : 0,
  };
  ...
  for (const p of trend180) {
    ...
    if (!monthlyAgg[monthKey]) monthlyAgg[monthKey] = { sales: 0, profit: 0 };
    monthlyAgg[monthKey].sales += p.sales;
    monthlyAgg[monthKey].profit += p.profit;
  }
